...
model.load_weights("model/model_weights.h5")  # Ganti dengan path yang sesuai
...
# # Fungsi untuk membuat Word Cloud dan menyimpannya di folder static
# def generate_wordcloud_and_save(text):
#     wordcloud = WordCloud(width=800, height=400, random_state=21, max_font_size=110).generate(text)

#     # Simpan gambar Word Cloud di folder static
...
def index():
    if request.method == 'POST':
        ...
        # Make sure prediction has the expected shape
        ...
        wordcloud_positive = WordCloud(width=400, height=400, background_color="white").generate(" ".join(unique_positive_words_only))
        wordcloud_positive.to_file("static/wordcloud_positive.png")

        # Buat Word Cloud untuk kata-kata negatif yang benar-benar unik
        wordcloud_negative = WordCloud(width=400, height=400, background_color="white").generate(" ".join(unique_negative_words_only))
